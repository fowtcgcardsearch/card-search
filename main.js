...
// GASからデータを取得するためのurl
const apiUrl = "https://script.google.com/macros/s/AKfycbwcw-XbUssgd9ebtRVn3yyhsGUMjX_cIyE6P2XFRXauBz1wcV44Xdq7XvolrxT9Ot8/exec?type=json"
...
/**
 * エンコードされた値を受け取って検索を実行する関数
 */
function searchByPropertyEncoded(type, encodedValue) {
  const value = decodeURIComponent(encodedValue);
  searchByProperty(type, value);
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 現在のURLを取得
  const isDevAttr = document.body.getAttribute('data-is-dev');
  const isDev = (isDevAttr === 'true'); // 文字列をBooleanに変換

  // URLの末尾が「/dev」で終わっている、またはURLに「/dev」が含まれているか判定
  if (isDev) {
    // テスト環境用のマイルドな警告バーを最上部に追加（黄色系・黒文字）
    const alertBar = document.createElement('div');
    alertBar.innerText = '🚧 テスト環境で動作中 🚧';
    alertBar.style.cssText = 'background: #fff3cd; color: #856404; border-bottom: 1px solid #ffeeba; text-align: center; font-weight: bold; padding: 6px 0; font-size: 14px; position: sticky; top: 0; z-index: 9999;';
...
let activeKeywordTooltip = null;
let keywordTooltipTimer = null;
